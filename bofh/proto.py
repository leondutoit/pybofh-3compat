...
def wash_response(bofh_response):
    ...
    if isinstance(bofh_response, str):
        ...
        for i in list(bofh_response.keys()):
...
class _Command(object):
    ...
    def args(self):
        ...
        if isinstance(self._args, str):
            ...
            return ret
        ret = self._fixed_args = [_Argument(self._bofh, x) for x in self._args]
        return ret
...
class Bofh(object):
    ...
    def _init_commands(self, reset=False):
        ...
        for key, value in list(cmds.items()):
